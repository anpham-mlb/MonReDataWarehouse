...
-- Check every address_id is distinct
...
-- ERROR 17: Year 2999 does not make sense and person_id 6000 is not in client table
select * from monre.visit where to_char(visit_date, 'yyyy') = 2999;
...
create table client_wish_dim as select distinct cw.feature_code, cw.person_id, f.feature_description
from feature_group_dim fg, client_wish cw, feature f
where fg.feature_group_list like ('%'||cw.feature_code||'%')
and f.feature_code = cw.feature_code;
...
create table person_dim as select distinct person_id, title, first_name, last_name
...
--------------------------------------------------------------------------------
-- FEATURE_GROUP_DIM
drop table feature_group_temp_dim purge;
create table feature_group_temp_dim as select distinct c.person_id,
listagg(cw.feature_code, ',') within group (order by cw.feature_code) as feature_group_list
from client c, client_wish cw
where c.person_id = cw.person_id
group by c.person_id;

select * from feature_group_temp_dim;

drop table feature_group_dim purge;
create table feature_group_dim as select feature_group_list from feature_group_temp_dim;

select * from feature_group_dim;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- AGENT_OFFICE_GROUP_DIM
-- Create table o1 containing total employees and size for each office
...
drop table person_office_group_dim purge;
create table person_office_group_dim as select distinct a.person_id,
...
select * from person_office_group_dim;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- LOCATION_DIM
...
select distinct r.property_id, p.property_type, p.property_no_of_bedrooms
...
create table rental_property_dim as select property_id, property_type, property_scale
...
select distinct to_char(rent_start_date, 'yyyy') || to_char(rent_start_date, 'mm') as rental_time_id,
...
create table client_temp_fact as select distinct c.person_id,
listagg(cw.feature_code, ',') within group (order by cw.feature_code) as feature_group_list,
c.max_budget, p.gender, count(distinct c.person_id) as number_of_clients
from client c, client_wish cw, person p
where c.person_id = cw.person_id
and p.person_id = c.person_id
group by c.person_id, c.max_budget, p.gender;
...
create table client_fact as select feature_group_list, gender, budget_category, number_of_clients
...
create table agent_fact as select a.person_id, 
listagg(ao.office_id, ',') within group (order by ao.office_id) as office_group_list,
...
drop table f1 purge;
create table f1 as
select distinct r.property_id, count(pf.feature_code) as number_of_features
...
-- Create table f2 containing total fee and number of rents
select * from rent;
drop table f2 purge;
create table f2 as
select distinct pe.person_id, r.property_id, 
to_char(r.rent_start_date, 'yyyymm') as rental_time_id,
(r.rent_end_date - r.rent_start_date)/30 as duration, p.address_id,
sum(r.price) as total_fee,
count(r.rent_id) as number_of_rents
from rent r, property p,person pe
where p.property_id = r.property_id
and pe.person_id = r.agent_person_id
and r.rent_start_date is not null
group by pe.person_id, r.property_id, to_char(r.rent_start_date, 'yyyymm'), p.address_id,
(r.rent_end_date - r.rent_start_date)/30
order by r.property_id;

alter table f2 add (rental_period varchar2(20));

update f2 set rental_period = 
(case
...
drop table rent_fact purge;
create table rent_fact as select f2.person_id, f1.property_id, f1.feature_category, f2.rental_time_id, 
f2.rental_period, f2.address_id, f2.total_fee, f2.number_of_rents
from f2, f1
where f1.property_id = f2.property_id
and f2.rental_time_id is not null
order by property_id, person_id;
...
select distinct pe.person_id, s.property_id, f1.feature_group_list, 
to_char(s.sale_date, 'yyyymm') as sale_time_id,
p.address_id, s.price as total_fee, f1.number_of_sales
from person pe, sale s, property_feature pf, property p, sale_temp_fact f1
where pe.person_id = s.agent_person_id
...
--------------------------------------------------------------------------------
-- VISIT_FACT
drop table visit_temp_fact purge;
create table visit_temp_fact as
select distinct to_char(v.visit_date, 'yyyymmdd') as visit_time_id, 
v.client_person_id as person_id, c.max_budget,
...
from visit v, client c
where v.client_person_id = c.person_id
group by to_char(v.visit_date, 'yyyymmdd'),  v.client_person_id, c.max_budget
order by number_of_properties desc;

alter table visit_temp_fact add (budget_category varchar2(20));

update visit_temp_fact set budget_category =
(case 
when max_budget > 0 and max_budget < 1000 then 'Low'
when max_budget > 1001 and max_budget < 100000 then 'Medium'
when max_budget > 100001 and max_budget < 10000000 then 'High'
...
create table visit_fact as select visit_time_id, person_id, budget_category,
number_of_properties, number_of_property_visits
from visit_temp_fact;
...
select distinct pa.advert_id, to_char(p.property_date_added, 'yyyymm') as advert_time_id,  
p.address_id, count(distinct pa.property_id) as number_of_properties
from property_advert pa, property p
where p.property_id = pa.property_id
group by pa.advert_id, to_char(p.property_date_added, 'yyyymm'), p.address_id
order by pa.advert_id, p.address_id;

select * from advert_fact;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- 3. Answer questions
--------------------------------------------------------------------------------
...
from person_office_group_dim p, agent_fact f
...
from agent_fact f, person_office_group_dim p
...
from rent_fact f, rent_property_dim rp, location_dim l, rental_time_dim rt
...
from rent_property_dim rp, rent_fact f
where f.property_id = rp.property_id
...
select * from (select s.person_id, p.title, p.first_name, p.last_name, l.suburb,
...
group by s.person_id, p.title, p.first_name, p.last_name, l.suburb 
order by sum(total_fee * number_of_sales) desc) fetch first 3 rows only;

-- Top 3 rent agents in Melbourne
select * from (select r.person_id, p.title, p.first_name, p.last_name, l.suburb,
...
and l.suburb like '%Melbourne%'
group by r.person_id, p.title, p.first_name, p.last_name, l.suburb 
...
select person_id, title, first_name, last_name, suburb, sum(total_revenue) as total_revenue
from 
(select person_id, title, first_name, last_name, suburb, total_revenue from a1 union all
select person_id, title, first_name, last_name, suburb, total_revenue from a2)
where rownum <= 3
group by person_id, title, first_name, last_name, suburb order by total_revenue desc;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
...
select * from (
select f.person_id, p.title, p.first_name, p.last_name, ao.size_group_list,
sum(f.total_earning) as total_earning,
dense_rank() over (order by sum(f.total_earning) desc) as earning_rank
from agent_fact f, person_dim p, person_office_group_dim ao
where f.person_id = p.person_id
and f.person_id = ao.person_id
and f.office_group_list = ao.office_group_list
and ao.size_group_list like '%Big%'
group by f.person_id, p.title, p.first_name, p.last_name, ao.size_group_list)
...
select * from rent_fact where rental_period = 'Meidum';
select * from rental_period_dim;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- REPORT 5
...
-- (the query is referring to the clients who had visit activities)
select f.budget_category, v.year, to_char(count(f.person_id)) as number_of_clients,
to_char(sum(count(f.person_id)) over (
order by year rows unbounded preceding)) as cum_number_of_clients
from visit_fact f, visit_time_dim v
where f.visit_time_id = v.visit_time_id
and f.budget_category = 'High'
group by f.budget_category, v.year;
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
-- REPORT 9
